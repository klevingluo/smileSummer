...
if 0 % Get points interactively
    ...
    for ix = 1:NPs
        ...
        scatter(Yp(ix),Xp(ix),2,'y','o','filled'); % display the point
        text(Yp(ix),Xp(ix),num2str(ix),'FontSize',2);
        
        axis(Hs);
        [Ys(ix),Xs(ix)]=ginput(1); % get the corresponding point
        scatter(Ys(ix),Xs(ix),2,'y','*'); % display the point
        text(Ys(ix),Xs(ix),num2str(ix),'FontSize',2);
    end
